...
export default (async function (req, res, router) {
  ...
  if (router.pathname.startsWith('/static/')) res.headers.set('cache-control', 'public, max-age=31536000, immutable');
  // Don't cache html files, otherswise cache for a week
  else if (!file.type.startsWith('text/html')) res.headers.set('cache-control', 'public, max-age=604800, immutable');
  res.body = file;
